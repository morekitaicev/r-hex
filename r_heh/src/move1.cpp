...
void MoveTo(const std_msgs::String::ConstPtr& msg) {
	...
	std::string command = "echo -n '1255 1255 1255' " + ports[2];
	sleep(1);
	while (current % 360 != 0) {
		int current = std::stoi(msg->data.c_str());
	...
	command = "rostopic pub setMove0 std_msgs/String \"data: ''\" --once";
	system(command.c_str());
}

void Move(const std_msgs::String::ConstPtr& msg) {
	ros::NodeHandle n;
	ros::Subscriber position0 = n.subscribe("pos1", 5, MoveTo);
}
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber sub = n.subscribe("setMove1", 5, Move);
